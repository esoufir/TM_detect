...
"""Geometrical classes and methods of the TM_detect tool."""
...
class Plane:
    ...
    def __init__(self, point: Point, normal: Vector):
        ...
        self.d = -(self.a * self.point.get_x() +
                   ...
                   self.c * self.point.get_z())  # https://mathworld.wolfram.com/Plane.html
    ...
    def is_above(self, point):
        ...
        """
        return True if (self.a * point.get_x() +
                        self.b * point.get_y() +
                        self.c * point.get_z() + self.d) > 0 else False
...
class Axis:
    ...
    def explore_axe(self, amino_acid_sequence, ref):
        """
        Explore the axis and decides if it's the better one encountered yet.
        If there are no more atoms between the planes,
        stop the exploration of this axis.
        ...
        for aa in amino_acid_sequence:
            ...
            if aa not in in_between_planes and aa.is_hydrophobic is False:
        ...
        # When no more atoms between the two planes, exiting the function
        if len(in_between_planes) == 0 or len(in_between_planes) >= len(amino_acid_sequence):
            return (False)
        ...
        return len(in_between_planes) > 0
    ...
    def explore_axe_bis(self, amino_acid_sequence,ref):
        ...
        if len(in_between_planes) is 0 or  len(in_between_planes) >= len(amino_acid_sequence):
            return (False)
...
def find_director_vector(point: Point, center_coordinate: Point):
    ...
    return Vector(center_coordinate.get_x() - point.get_x(), 
                  center_coordinate.get_y() - point.get_y(),
